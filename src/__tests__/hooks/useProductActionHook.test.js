...
describe('Product action hook', () => {
  ...
  test.skip('failed to fetch products from api: 404', async () => {
    axios.get.mockRejectedValue(new NotFoundError('failed to fetch product from api'))
    ...
    await waitFor(() =>
      expect(axios.get).rejects.toThrow(new NotFoundError('failed to fetch product from api'))
    )
    await waitFor(() => expect(axios.get).rejects.toThrowError('failed to fetch product from api'))
  })

  test.skip('failed to fetch products from api - mockRejectedValue: 404', async () => {
    const mocked = jest.mocked
    mocked(axios.get).mockRejectedValue(new Error('failed to fetch product from api'))
